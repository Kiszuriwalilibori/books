...
let SingleBook = () => {
  const singleBookData = useSelector(state => state.details.currentBookWithDetailedInfoData, shallowEqual);
  const prevButtonVisible = useSelector(state => state.details.prevDetailedBookButtonVisible, shallowEqual);
  const nextButtonVisible = useSelector(state => state.details.nextDetailedBookButtonVisible, shallowEqual);
  
  const dispatch = useDispatch();
  const requirePrevious = useCallback(
    debounce(() => dispatch({ type: "details/PREVIOUS_DETAILS_SHOW" }), 200),
    [dispatch]
  );
  const requireNext = useCallback(
    debounce(() => dispatch({ type: "details/NEXT_DETAILS_SHOW" }), 200),
    [dispatch]
  );
